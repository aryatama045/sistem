...
$(document).ready(function() {
    ...
    manageKas = $('#manageKas').DataTable({
        ...
        'scrollX': true,
        'ajax': {
            'url': base_url + 'leaves/kas_kecil/fetchApprovalFinance',
            'type': 'POST',
        },
        'order': [0, 'DESC'],
        'columnDefs': [{
    ...
    manageHistory = $('#manageHistory').DataTable({
        ...
        'scrollX': true,
        'ajax': {
            'url': base_url + 'leaves/kas_kecil/fetchDataHistoryFin',
            'type': 'POST',
            'data': function(data) {
                data.no_req = $('#no_req').val();
                data.nama_req = $('#nama_req').val();
            },
        },
        'order': [0, 'DESC'],
        'column': [{
    ...
    $('#no_req').on('keyup', function(event) { // for text boxes
        manageHistory.ajax.reload(); //just reload table
    });

    $('#nama_req').on('keyup', function(event) { // for text boxes
        manageHistory.ajax.reload(); //just reload table
    });
});
...
function detail(id) {
    ...
    $.ajax({
        url: base_url + 'leaves/kas_kecil/get_history/' + id,
        type: "GET",
        dataType: "JSON",
...
function reject(id) {
    //Ajax Load data from ajax
    $.ajax({
        url: base_url + 'leaves/kas_kecil/get_request/' + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
