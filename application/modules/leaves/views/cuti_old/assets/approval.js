...
$(document).ready(function() {
    ...
    manageApproval = $('#manageApproval').DataTable({
        ...
        'scrollX': true,
        'ajax': {
            'url': base_url + 'leaves/cuti/fetchDataApproval',
            'type': 'POST',
        },
        'order': [0, 'DESC'],
        'columns': [
    ...
    $('.search-input-text').on('keyup', function(event) { // for text boxes
        ...
        if (keycode == 13) {
            manageApproval.columns(i).search(v).draw();
        }
    });
    ...
    manageApprovalHistory = $('#manageApprovalHistory').DataTable({
        // 'orderCellsTop': true,
        // 'fixedHeader': true,
        'processing': true,
        'serverSide': true,
        'scrollX': true,
        'ajax': {
            'url': base_url + 'leaves/cuti/fetchDataHistory',
            'type': 'POST',
        },
        'order': [0, 'DESC'],
        'columns': [{
    ...
    $("#manageApprovalHistory_filter").css("display", "none"); // hiding global search box
    $('.search-input-text').on('keyup', function(event) { // for text boxes
        var i = $(this).attr('data-column'); // getting column index
        var v = $(this).val(); // getting search input value
        var keycode = event.which;
        if (keycode == 13) {
            manageApprovalHistory.columns(i).search(v).draw();
        }
    });
    $('#manageApprovalHistory tbody').on('click', 'tr td.details-control', function() {
        var tr = $(this).closest('tr');
        // var row = manageApproval.row(tr);
        var row = manageApprovalHistory.row(tr);
...
function tampil_detail_approved(d) {
    // d is the original d object for the row var val;
    if (d.secondary.length == null) {
        return "There are no datas";
    }
