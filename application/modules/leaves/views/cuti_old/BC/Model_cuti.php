...
class Model_cuti extends CI_Model
{
	...
	public function getDataTambahan($biodataid,$dates){
		$dates		= date('Y-m-d');
		$sql 	= " SELECT  ct.biodata_id,ct.saldo_cuti_tambahan_id,ct.saldo_awal,
	...
	public function getMasaBerlakuTambahan($biodataid,$dates){
		...
		AND is_batal <> 1
		AND sisa_cuti > 0
		AND NOW() >= DATE(tgl_mulai_berlaku) AND NOW() - INTERVAL 1 DAY <= DATE(tgl_akhir_berlaku)
		ORDER BY tgl_akhir_berlaku ASC";
		$query 	= $this->hrd->query($sql);
		// die($this->hrd->last_query());
		return 	$query->result_array();
	...
	/* Create & Approce Action */
		...
		public function create()
		{
			...
			/* Cek saldo cuti normatif */
				$biodataids        = $this->input->post('biodata_id');
				$cek_cuti 			= $this->getDataNormatif($biodataids);
			/* // END Cek saldo cuti normatif */
			...
			/* Condition */
				...
				#sisa saldo
				if($this->input->post('sisa_normatif')!=='0'){
					$jml_normatif = $count_d;
				} else{ $jml_normatif ='0'; }
				if($this->input->post('sisa_bonus')!=='0'){
					$jml_bonus = $count_d;
				}else{ $jml_bonus ='0'; }
				if($this->input->post('sisa_tambahan')!=='0'){
				...
				if($potong_cuti =='NORMATIF'){
					if($jumlah_hari =='1'){
						if($cek_cuti['sisa_normatif'] == null || $cek_cuti['sisa_normatif'] <= '0'){
							$status_absen 		= 'CG';
							// $status_absen_id 	= '000000000044';
						}else{
							$status_absen 		= 'CN';
							// $status_absen_id 	= '000000000012';
						}
					}else{
							$status_absen 		= 'CH';
					...
					if($cek_cuti['sisa_normatif'] == null){
						$generate_saldo = array(
							...
							'tgl_mulai_berlaku' 		=> date('Y-01-01'),
							'tgl_akhir_berlaku' 		=> date('Y-12-31'),
							'tgl_input'    				=> date('Y-m-d H:i:s'),
						);
						$this->hrd->insert('hrd_all.trn_saldo_cuti_normatif', $generate_saldo);
					}


					/* Cek Saldo Normatif */
						$cek_saldo_normatif = $this->getDataNormatif($biodataids);
						$tgl_akhir_berlaku 	= $cek_saldo_normatif['tgl_akhir_berlaku'];
						$tgl_awal_berlaku 	= $cek_saldo_normatif['tgl_mulai_berlaku'];
					/* //Cek Saldo Normatif */
				}
				#potong cuti bonus
				...
				for($x = 0; $x < $count_tgl_tdk_masuk; $x++) {
					...
					if($potong_cuti =='NORMATIF'){
						...
						if($jumlah_hari == '1'){
							...
							$saldo_n = $sisa_cuti_normatif - $jumlah_hari;

							if($sisa_cuti_normatif <= '0'){
								$status_absen_d 		= 'CG';
							}else{
								$status_absen_d 		= 'CN';
							}
		...
		public function ApproveAction()
		{
			...
			if($urutan_approval==$count){
				...
				if($potong_cuti=='BONUS'){
					...
					for($x = 0; $x < $count_tgl_tdk_masuk; $x++) {
						...
						$items = array(
							...
							'jml_awal' 				=> $cek_cuti['sisa_normatif'],
							'jml_akhir' 			=> $jml_awal,
							'pic_input'         	=> $this->input->post('biodata_id'),
							'tgl_input'    			=> date('Y-m-d H:i:s'),
						);
				...
				// tesx($potong_cuti,$jml_ambil, $sisa_cuti, $data_absensi);
