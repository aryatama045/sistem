...
class Model_ijin extends CI_Model
{
	...
	function __construct()
	{
		...
		$this->pos = $this->load->database('pos5ot_master',TRUE);
	}
	...
	public function getStatusCuti(){
		...
		FROM hrd_all.mst_status_absensi WHERE kode_status_absensi IN ('IJ','SK','ID','IP') ORDER BY ket_status_absensi ASC";
	...
	public function getJamShift($biodata_id,$nip,$gol_absen,$tgl_ijin,$is_shift,$kd_store){
		if($is_shift=='0' || $is_shift==NULL){
			...
			$kode = array('SP_HO','OT_HO','TOL_HO','TOL_JK','TOL_PL','TOL_SB','TOL_YG');
			$st = in_array(trim($kd_store) , $kode);

			if($is_shift =='1' &&  $st == true){
				$sql = "SELECT c.jam_shift_masuk shift_masuk, c.jam_shift_keluar shift_keluar,c.jam_masuk,c.jam_keluar,b.jam_mulai_istirahat,b.jam_akhir_istirahat,
					SUBTIME(c.jam_shift_keluar,c.jam_shift_masuk)lama_shift,TIME_TO_SEC(TIMEDIFF(c.jam_masuk,c.jam_shift_masuk))selisih,
					TIME_TO_SEC(SUBTIME(SUBTIME(c.jam_shift_keluar,c.jam_shift_masuk),SUBTIME(b.jam_akhir_istirahat,b.jam_mulai_istirahat)))detik_kerja_min_istirahat,
					SUBTIME(SUBTIME(c.jam_shift_keluar,c.jam_shift_masuk),SUBTIME(b.jam_akhir_istirahat,b.jam_mulai_istirahat))jam_kerja_min_istirahat,
				...
				$sql = "SELECT  a.nip, a.tanggal, a.kd_store, a.kode_shift, jam_in jam_shift_in, jam_out jam_shift_out, istirahat, TIME(c.created_on) jam_in
					,SUBTIME(jam_out,jam_in)lama_shift,TIME_TO_SEC(TIMEDIFF(TIME(c.created_on),jam_in))selisih
					,CASE
					WHEN TIME_TO_SEC(TIMEDIFF(TIME(c.created_on),jam_in)) > 7200 THEN 0
					WHEN TIME_TO_SEC(TIMEDIFF(TIME(c.created_on),jam_in)) <= 0 THEN 2
					WHEN TIME_TO_SEC(TIMEDIFF(TIME(c.created_on),jam_in)) IS NULL THEN 3
					ELSE 1 END boleh
					FROM pos5ot_transaction.trn_shift a
					LEFT JOIN pos5ot_master.mst_shift b ON a.kode_shift = b.kode_shift
					LEFT JOIN ots_web_transaction.app_attendance_daily c
					ON a.nip = c.nip AND a.tanggal = DATE(created_on) AND modes=1
					WHERE a.nip='$nip' AND tanggal='$tgl_ijin'
				";
				$query 	= $this->pos->query($sql);
				// die($this->pos>last_query());
				return 	$query->row_array();
	...
	public function create()
	{
		...
		for($x = 0; $x < $count_tgl_tdk_masuk; $x++) {
			...
			if($this->input->post('status_absensi_id') == '000000000061'){
				$getJamShift= $this->getJamShift($biodata_id,$nip,$gol_absen,$tgl_ijin,$is_shift,$kd_store);

				#condition cek pengajuan 0=tidak boleh, 1=boleh, 2=tidak telat, 3=belum absen
				if($getJamShift['boleh'] == '0' || $getJamShift['boleh'] == '2' || $getJamShift['boleh'] == '3' || $getJamShift == NULL){
					...
					else if($getJamShift == NULL){
						$this->session->set_flashdata('error', $tgl_ijin.' Tidak ada Jam Absen / Jam shift ');
					}
        ...
        /* Manage Image */
			...
			if(!empty($this->upload->do_upload('file_1'))){
			...
			if(!empty($this->upload->do_upload('file_2'))){
			...
			if(!empty($this->upload->do_upload('file_3'))){
		...
		tesx($tgl_ijin,$nip,$getShift, !empty($getJamShift),$data_header, $log_detail,$data_dokumen, $data_posting);
	...
	*/
	...
	public function getHeaderHistory($no_dok_h = null)
	{
		if($no_dok_h) {
			$sql = "SELECT a.*,k.*,r.tgl_reject tgl_reject_hrd,r.alasan_reject, s.nama_lengkap pic_reject, ket_status_absensi,kode_status_absensi
			FROM hrd_all.trn_tidak_masuk_h a
	...
	public function getVerifikasi1($nip , $search_no = "", $length = "", $start = "", $column = "", $order = "")
	{
		...
		if($nips==$sql_pic['nip']){
			...
			if($sql_pic['level'] == '2'){
				$where = 'AND k.kd_store IN ("SP_HO","OT_HO","OT_ME1","TOL_HO","TOL_JK","TOL_PL","TOL_SB","TOL_YG","TOL_BD")';
			} else if($sql_pic['level'] == '3') {
				$where = 'AND k.kd_store NOT IN ("SP_HO","OT_HO","OT_ME1","TOL_HO","TOL_JK","TOL_PL","TOL_SB","TOL_YG","TOL_BD")';
			}
	...
	public function getVerifikasi2($nip , $search_no = "")
	{
		...
		if($nips==$sql_pic['nip']){
			if($sql_pic['level'] == '2'){
				$where = 'AND k.kd_store IN ("SP_HO","OT_HO","OT_ME1","TOL_HO","TOL_JK","TOL_PL","TOL_SB","TOL_YG")';
			} else if($sql_pic['level'] == '3') {
				$where = 'AND k.kd_store NOT IN ("SP_HO","OT_HO","OT_ME1","TOL_HO","TOL_JK","TOL_PL","TOL_SB","TOL_YG")';
			}
	...
	public function getVerifikasiHistory1($nip , $search_no = "",$search_nama="", $length = "", $start = "", $column = "", $order = "")
	{
		...
		$where_search_nama = "";
		if($search_nama !== ""){
			$where_search_nama = "AND nip LIKE '%".$search_nama."%' OR nama_lengkap LIKE '%".$search_nama."%' ";
		}

		$where_search_no = "";
		if($search_no !== ""){
			$where_search_no = "WHERE no_dok_history = '".$search_no."'";
		}
	...
	public function getVerifikasiHistoryData($nip , $search_no = "",$search_nama="", $length = "", $start = "", $column = "", $order = "")
	{
		$nips 				= $this->session->userdata('nama_login');
		$sql_pic 			= $this->getDataPicDewaHrd();
		$where_search_nama = "";
	...
	public function ApproveAction()
	{
		...
		// tesx($nip, $sql_pic, $urutan_approval, $count, $status_abesensi);
	...
	public function rejectApp($h_id,$pic,$nip,$ket)
	{
		...
		$no_dok 			= $sel_dok['no_dok_tdk_masuk'];
		$urutan_approval 	= $sel_dok['urutan_approval'];
		$count				= $sel_dok['jml_app'];
		...
		if($urutan_approval==$count){
			...
			$data_posting = array(
				...
				'tgl_rej_'.$urutan_approval.'' 			=> date('Y-m-d'),
				'status' 								=> $urutan_approval,
				'status_dokumen' 						=> 'R',
