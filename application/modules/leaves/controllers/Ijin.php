...
class Ijin extends Admin_Controller  {
	...
	public function create()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			// $file_info = $this->upload->do_upload('file_1');
			// $post_img1 = $this->upload->data();

			// tesx($post_img1['file_name'], 'test');
	...
	*/
	...
	public function detail($no_dok_h)
	{
		...
		$this->data['status_hrd'] = $this->Model_ijin->getApp3rd($header_data['tdk_masuk_h_id']);
		// tesx($this->data['status_hrd']);
		$this->render_template('ijin/detail_new',$this->data);
	}
	...
	public function fetchDataApprovalHistory()
	{
		...
		if($data){
			foreach ($data as $key => $value) {
				...
				$file1 = '	<div>
									<a data-fancybox="gallery" data-src="'.base_url('upload/ijin/'.$value['file_1']).'">
	...
	public function fetchDataVerifikasiHistory()
	{
		...
		if($data){
			...
			foreach ($data as $key => $value) {
				...
				$buttons = '';
				$buttons .= ' <a class="btn btn-primary mb-1" href="#" onclick="detail_doc_history('."'".$value['no_dok_cuti']."'".')">
				<i class="fa fa-eye"></i> '.$value['no_dok_cuti'].'</a>';
					$arr_result = array(
						$buttons,
						$value['tgl_dok_cuti'],
						$value['nip'],
	...
	public function detail_doc_history($id)
	{

		$nip 		= $this->session->userdata('nama_login');
		$search_no  = $id;

		$header = $this->Model_ijin->getVerifikasiHistory1($nip, $search_no, '','1','0','','');

		
		$data 	= $this->Model_ijin->getDetailHD($header[0]['id_doc'],$header[0]['jn']);

		$app_data 	= $this->Model_cuti->getDataPosting($header[0]['id_doc']);
		$urutan_app	= $this->Model_cuti->urutanApp($header[0]['biodata_id']);

		// tesx($id, $header[0]['id_doc'], $header);
		if($data){
			$output['potong'] = $header[0]['jn'];
			$output['nip'] = $header[0]['nip'].'-'.$header[0]['nama_lengkap'];
			$output['keterangan'] = $header[0]['catatan'];
			foreach ($data as $key => $value) {
				$output['data'][$key] = array(
					$value['tgl_ijin'],
					$value['nama_hari'],
	...
	public function getAbsenMasuk(){
		...
		if ($datas == null){
			$data = array( 'jm' => "Tidak ada data absen");
		}else{
	...
	public function getAbsenMasukUser(){
		$tgl_ijin 	= $this->input->post('tgl',TRUE);
		$tgl	 	= date('Y-m-d', strtotime($tgl_ijin));
			...
			if(!empty($_FILES['lampiran'.$val]['name'])){
				...
				if($this->upload->do_upload('lampiran'.$val)){
					...
					if($_FILES['lampiran'.$val]['size'] <= '1565000'){
						...
						redirect('leaves/ijin/detail/'.$tdk_masuk_h_id , 'refresh');
					}else {
						$config['quality']	= '100%';
						$config['width']	= '100%';
						$config['height']	= '100%';
