...
class Model_report extends CI_Model
{
	...
	public function getDataAbsen($tanggal1,$tanggal2,$divisi_id="",$dept_id="",$kd_store="",$search_no = "", $length = "", $start = "", $column = "", $order = "")
	{
		...
		if($kd_store !=""){ $where .= " AND kd_store = '".$kd_store."'";}
	...
	public function getJumlahDataAbsen($tanggal1,$tanggal2,$divisi_id="",$dept_id="",$kd_store="", $search_no="")
	{
		$where = "";
		if($search_no != ""){ $where .= " AND c.nip = $search_no";}
		if($divisi_id !=""){ $where .= " AND d.divisi_id = $divisi_id";}
	...
	public function getDataAbensiStatus($tanggal1,$tanggal2,$divisi_id,$dept_id,$kd_store,$status_absensi,$karyawan,$search_no = "", $length = "", $start = "", $column = "", $order = "")
	{
	...
	public function getJumlahDataAbsensiStatus($tanggal1,$tanggal2,$divisi_id,$dept_id,$kd_store,$status_absensi,$karyawan)
	{
	...
	public function getDataApprovalPersonil($divisi_id, $dept_id, $length = "", $start = ""){
		$sql = "SELECT
	...
	public function getJumlahDataApprovalPersonil($divisi_id, $dept_id){
		...
		LEFT JOIN hrd_all.mst_divisi h ON g.kode = h.kode_divisi
		LEFT JOIN hrd_all.biodata_pekerjaan_d i ON e.biodata_id = i.biodata_id
		WHERE IFNULL(e.nama_lengkap,'')<>'' AND h.divisi_id ='".$divisi_id."' AND f.dept_id='".$dept_id."'
		ORDER BY nama,e.nip, urutan_approval";
